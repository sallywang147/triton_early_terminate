...
def replace_llama_qkv_and_mlp_fc1_with_triton(model, BM=32, BN=64, BK=32, num_warps=4, num_stages=3):
    dev = next(model.parameters()).device
    replaced = {"q": 0, "k": 0, "v": 0, "gate": 0}

    for _, module in model.named_modules():
        # Attention QKV
        if hasattr(module, "q_proj") and isinstance(getattr(module, "q_proj"), nn.Linear):
            module.q_proj = TritonLinearBaseline(module.q_proj, BM=BM, BN=BN, BK=BK, num_warps=num_warps, num_stages=num_stages).to(dev)
            replaced["q"] += 1

        if hasattr(module, "k_proj") and isinstance(getattr(module, "k_proj"), nn.Linear):
            module.k_proj = TritonLinearBaseline(module.k_proj, BM=BM, BN=BN, BK=BK, num_warps=num_warps, num_stages=num_stages).to(dev)
            replaced["k"] += 1

        if hasattr(module, "v_proj") and isinstance(getattr(module, "v_proj"), nn.Linear):
            module.v_proj = TritonLinearBaseline(module.v_proj, BM=BM, BN=BN, BK=BK, num_warps=num_warps, num_stages=num_stages).to(dev)
            replaced["v"] += 1

        # MLP first FCs before activation (SwiGLU)
        if hasattr(module, "gate_proj") and isinstance(getattr(module, "gate_proj"), nn.Linear):
            module.gate_proj = TritonLinearBaseline(module.gate_proj, BM=BM, BN=BN, BK=BK, num_warps=num_warps, num_stages=num_stages).to(dev)
            replaced["gate"] += 1


    print("Replaced LLaMA layers with TritonLinearBaseline:", replaced)
    return model
...
def main():
    ...
    # Patch QKV + MLP gate/up
    replace_llama_qkv_and_mlp_fc1_with_triton(model, BM=32, BN=64, BK=32, num_warps=4, num_stages=3)
